...
fn test_permission_manager_creation() {
    ...
    let result = nexus_agent::permissions::PermissionManager::new();
    assert!(
        result.is_ok(),
        "PermissionManager should create successfully"
    );
}

#[test]
fn test_permission_defaults() {
    let manager = nexus_agent::permissions::PermissionManager::new().unwrap();

    // Default should be Ask for unknown tools (use unique name to avoid conflicts)
    let unique_tool = format!(
        "test_tool_unique_{}",
    ...
    assert_eq!(level, nexus_agent::permissions::PermissionLevel::Ask);
}

#[test]
fn test_permission_persistence() {
    let mut manager = nexus_agent::permissions::PermissionManager::new().unwrap();

    // Set a permission
    manager
        .set_permission(
            "test_tool_always".to_string(),
            nexus_agent::permissions::PermissionLevel::Always,
        )
        .unwrap();

    // Verify it's set
    let level = manager.check_permission("test_tool_always");
    assert_eq!(level, nexus_agent::permissions::PermissionLevel::Always);

    // Create a new manager (simulating restart)
    let manager2 = nexus_agent::permissions::PermissionManager::new().unwrap();
    ...
    let mut manager3 = nexus_agent::permissions::PermissionManager::new().unwrap();
    manager3
        .set_permission(
            "test_tool_always".to_string(),
            nexus_agent::permissions::PermissionLevel::Ask,
        )
        .unwrap();
}
...
fn test_session_only_permissions() {
    ...
    manager
        .set_permission(
            ...
            nexus_agent::permissions::PermissionLevel::Once,
        )
        .unwrap();

    // Should be available in same manager
    let level = manager.check_permission("test_tool_once");
    assert_eq!(level, nexus_agent::permissions::PermissionLevel::Once);
...
fn test_permission_storage_location() {
    use nexus_agent::permissions::PermissionManager;
