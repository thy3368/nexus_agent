...
use nexus_agent::agent::domain::agent::Agent;
use nexus_agent::Config;
use nexus_agent::model::openai::OpenAIProvider;
use nexus_agent::tools::{file_ops, git_ops, search_ops, shell, web_ops, ToolRegistry};
...
fn handle_doctor(config: &Config) -> anyhow::Result<()> {
    ...
    println!("✓ Binary version: {}", nexus_agent::VERSION);
...
async fn handle_agent(task: &str, config: Config) -> anyhow::Result<()> {
    ...
    let model: Box<dyn nexus_agent::model::LanguageModel> = match provider.as_str() {
        "ollama" => {
            ...
            Box::new(nexus_agent::model::ollama::OllamaProvider::new(
                base_url,
                api_key,
    ...
    let permission_manager = std::sync::Arc::new(std::sync::Mutex::new(
        nexus_agent::permissions::PermissionManager::new()?
    ));
...
async fn handle_chat(mut config: Config) -> anyhow::Result<()> {
    ...
    println!("\x1b[32m    PromptLine v{} (Rust) - Agentic AI CLI\x1b[0m", nexus_agent::VERSION);
    ...
    let helper = nexus_agent::repl::ReplHelper::new();
    ...
    loop {
        ...
        let mut command_handler = nexus_agent::commands::CommandHandler::new(config.clone(), permission_manager);
        ...
        loop {
            ...
            match readline {
                Ok(line) => {
                    ...
                    if let Some(command) = nexus_agent::commands::CommandHandler::parse(input) {
                        match command_handler.execute(command) {
                            Ok(output) => {
                                println!("{}", output.message);
                                match output.action {
                                    nexus_agent::commands::CommandAction::Quit => return Ok(()),
                                    nexus_agent::commands::CommandAction::ClearHistory => {
                                        agent.conversation_history.clear();
                                        println!("✓ Session cleared");
                                    }
                                    nexus_agent::commands::CommandAction::ReloadAgent => {
                                        println!("↻ Reloading agent...");
                                        reload_requested = true;
                                        break; // Break inner loop to reload
                                    }
                                    nexus_agent::commands::CommandAction::None => {}
                                }
                                continue;
                            }
                            Err(e) => {
                    ...
                    match agent.execute_task(input.to_string()).await {
                        Ok(result) => {
                            ...
                            agent.conversation_history.push(nexus_agent::model::MMessage::assistant(response_content.clone()));
                        }
                        Err(e) => {
